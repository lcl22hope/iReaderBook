...
@interface HeartSoupViewController ()

@end

@implementation HeartSoupViewController
...
-(UIView*)newsHeaderView
{
    if (!_newsHeaderView) {
        ...
        fromLabel.text = [NSString stringWithFormat:@"来自于:%@",@"清华南都"];
        ...
        likeView.loveCount.text = [NSString stringWithFormat:@"%d次",368];
...
-(UIImageView*)newsImgView{
    if (!_newsImgView) {
        ...
        _newsImgView.image = [UIImage imageNamed:@"心灵鸡汤"];
    }
    return _newsImgView;
}
...
-(UITextView*)newsDetailView{
    if (!_newsDetailView) {
        ...
        NSError* err = nil;
        NSString* filePath = [[NSBundle mainBundle] pathForResource:@"content2" ofType:@"txt"];
...
-(commentViewINDetail*)commentDetail
{
    if (!_commentDetail) {
        ...
        _commentDetail.date.text = @"2014-09-09";
